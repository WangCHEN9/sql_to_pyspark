...
def fn_genSQL(data, query):
    v_fn_from = (
        v_fn_where
    ) = v_fn_groupby = v_fn_agg = v_fn_select = v_fn_orderby = v_fn_limit = ""
...
def converter_main(query):
    v_parse = parse(query)
    v_json = json.loads(json.dumps(v_parse, indent=4))
    return fn_genSQL(v_json, query=query)
